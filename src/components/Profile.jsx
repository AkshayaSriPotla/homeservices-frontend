...
export const Profile = ({ isLoggedIn, userMobile }) => {
  ...
  useEffect(() => {
    if (isLoggedIn && userMobile) {
      const fetchUserInfo = async () => {
        try {
          const response = await fetch(`http://localhost:8082/api/login/${userMobile}`); // Updated endpoint
          if (response.ok) {
  ...
  useEffect(() => {
    if (isLoggedIn && activeTab === 'orders') {
      const fetchBookings = async () => {
        try {
          const response = await fetch(`http://localhost:8082/api/bookings/user/${userMobile}`);
          if (response.ok) {
            const data = await response.json();
  ...
  const cancelBooking = async (bookingId) => {
    ...
    try {
      const response = await fetch(`http://localhost:8082/api/bookings/${bookingId}`, {
        method: 'DELETE',
      });
